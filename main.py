...
class VisionProfessionalSystem:
    ...
    def refresh_display(self):
        ...
        # 模式列表更新
        mode_names = [
            "1.标准视力表", "2.散光钟图", "3.红绿对比图", 
            "4.蜂窝图1", "5.蜂窝图2", "6.交叉十字线", "7.Worth四点图"
        ]
        self.status_bar.config(text=f"模式: {mode_names[self.mode-1]} | 方向左右键:切换模式 | 方向上下键: {'切换行' if self.mode==1 else '缩放'} | 快捷键: 1-7")

        # 特殊背景色逻辑：红绿图(3)和Worth四点图(7)通常使用黑色背景
    ...
    def draw_vision_chart(self, w, h):
        ...
        for i, idx in enumerate(indices):
            if 0 <= idx < len(VISION_DATA):
                ...
                text_margin = 170 
                safe_l = text_margin + (px_size / 2)
                safe_r = w - (px_size / 2) - 60
                ...
                if usable_w > 0:
                    if num > 1:
                        step = usable_w / (num - 1)
                        for n in range(num):
                            self.draw_single_e(safe_l + (n * step), y_pos, px_size)
                    else:
                        self.draw_single_e(safe_l + usable_w/2, y_pos, px_size)
